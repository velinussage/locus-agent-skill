...
## Choose the property decision first

Start with `locus_suggest_workflow` for a broad question, then `locus_lane_availability` for the exact place. Use the records below as evidence for the person's decision, not as a verdict. Skip unavailable lanes; use the live catalog for current names and schemas.

| Decision | Free record evidence to check first |
|---|---|
| Owning costs or a revaluation notice | `locus_tax_calendar` and `locus_appeal_filing_guide` for statutory rules, official county pointers, and filing steps; confirm current-cycle dates, then use `locus_property_tax_estimate` where supported. Use `locus_appeal_window` with a notice date if deadline arithmetic is needed. Paid assessment or owner-cost follow-ups only if the free evidence leaves a question and the user approves. |
| Buying | `locus_place_facts`, parcel records, zoning, flood, transfers, and development cases for the exact property. Check each returned source before comparing places; do not pick a winner. |
| Renting | `locus_rental_registration_check` where wired, plus parcel, flood, and nearby official records. A no-match is not proof of legal permission or habitability. |
| Leasing | Parcel, zoning, permits, and nearby development records for the proposed premises; use workplace context only if jobs near the site matter. Jobs are not customer demand. |
| Lending | Parcel, zoning, transfers, flood, and dated area lending or house-price records where available. Area aggregates do not value or qualify the subject or a borrower. |
| Commercial property financing or ownership | Identify the exact commercial property and check parcel facts first. If the paid CMBS lanes appear in the live index, use `locus-cmbs-loan-context` for its filed loan and maturity, then `locus-cmbs-maturity-wall` only when county-level timing matters. Use the distinct area-context lanes described below only when they answer the user's question; none establish the property's loan or value. |
...
## Commercial property and public CMBS filings

- For a lender, investor, or commercial-property owner asking about this property's debt, first identify the exact address and confirm the live paid catalog lists `locus-cmbs-loan-context`. After payment approval, read the filed trust, SEC filing URL, asset number, reporting period end, loan maturity date, top-three largest-lease expirations, and any special-servicer transfer date together. Count a 24-month maturity window **from the filing's reporting period end**, not today. Verify later loan or lease changes with the current servicer and property records; a filed transfer is not a default verdict.
- When the decision concerns a **county's** filed debt timing, check the live catalog for `locus-cmbs-maturity-wall` and use a five-digit county FIPS or an exact address. Read the 6-, 12-, 18-, and 24-month buckets and property types against the returned reporting period. Do not treat a county aggregate as this property's loan status, or add the buckets without checking their boundaries.
- Add free `locus_hmda_tract_context` for floor-protected **home-purchase** lending activity and `locus_fhfa_area_hpi` for historical **residential** house-price index context only when useful; neither describes a commercial loan or values the property. `locus_cbp_zip` is ZIP-level industry establishment context, not the property's tenants or revenue. `locus_address_sales_tax` is a published address rate only in supported states, not the tax due on a lease. `locus_eviction_filings` is area-level filing history, never a tenant, landlord, or property screening signal. Skip these unrelated lanes rather than building a generic dossier.
- **Coverage is partial.** CMBS snapshots cover publicly registered conduit and single-asset/single-borrower trusts filing SEC ABS-EE Exhibit 102, not bank balance-sheet loans, agency multifamily loans, or private-label deals that do not file. A no-match, `not_loaded`, `out_of_coverage`, or `unavailable` result is not proof of no loan or no maturities. These are paid-only lanes; neither belongs on the free tool rail. A no-data diagnostic returns `charged:false`. Present dated filing facts with their SEC links, not a credit rating, distress label, appraisal, recommendation, or prediction.
...
- **Check every fact's place and vintage before presenting it.** Confirm the returned state, county, and municipality against the question; name the record's date or layer vintage. If a tax rate belongs to a same-named county in another state, do not present it as this place's rate. If a layer was last updated years ago, label it historical, not current; if the place or vintage cannot be confirmed, say so and verify with the official source.
...
2. **A reassessment notice or owner cost question:** check free `locus_tax_calendar` and `locus_appeal_filing_guide` first, then a supported tax estimate. If a question remains and the user approves, `locus-owner-cost-review` at `$0.25` adds a recorded tax or assessment trajectory, cited programs and dates, and ready follow-up calls. The price includes one conservatively priced Turnkey signature. It does not infer the cause or determine eligibility.
...
| `POST /api/locus-cmbs-loan-context` | `$0.10` | Lender, investor, or commercial-property owner needs this exact property's filed CMBS loan context, maturity, top-three lease expirations, and special-servicer transfer with its SEC filing. Publicly registered ABS-EE trusts only. | Not loaded, unresolved, out-of-coverage, unavailable, or no match returns `charged:false`; no match does not establish no loan. No appraisal, loan verdict, or recommendation. |
| `POST /api/locus-cmbs-maturity-wall` | `$0.25` | County-level public CMBS balance and loan maturity timing by property type and special-servicer transfer, from the filed reporting period. Accepts an exact address or a five-digit county FIPS. | Missing/unavailable snapshot or unresolved county returns `charged:false`; coverage excludes non-filing loans. Never a property-level credit conclusion. |
| `POST /api/locus-practitioner-read` | `$0.29` | Agent wants what an experienced land and property analyst reads into the public records for one address: which patterns are present (repeat transfers, permit with no closeout, flood zone next to county claims history, zoning headroom, soil limits, enacted vs pending ordinance), the record that confirms each, and one plain-language summary. Optional `question` sets the audience. | Flat price on every call; an empty reading set is still paid. The response's `catalogue` block says how many readings are currently evaluated vs gated off pending a slot adapter (gate list with reasons in `readingsNotEvaluated`). Readings are framings to test with a named confirming record, never findings; no value, no safe/unsafe, nothing about a person. `partial` names the record Locus does not carry (deed type, parcel-level liens, panel revision history). Unresolved address, engine outage, or a prose guardrail failure returns `charged:false`. |
...
| Address sales-tax rate | `locus_address_sales_tax` | `{ "address": "123 Pike St, Seattle, WA 98101" }` | Washington and California state revenue agency address-level rates only. Other states return `out_of_coverage`; unresolved addresses return `unavailable`. Confirm the effective date and taxable sale with the state; not tax advice. |
...
| Tract and ZIP house-price index series | `locus_fhfa_area_hpi` | `{ "address": "..." }` or `{ "latitude": 35.22, "longitude": -80.84 }` | Latest published FHFA annual tract and ZIP5 index observations from separately loaded official snapshots, with each lane's coverage and release date. Area context, not a property valuation; unloaded data stays `not_loaded`. |
...
| HMDA tract lending context | `locus_hmda_tract_context` | `{ "address": "..." }` | Published tract-level home-purchase counts and median reported property value where a floor-protected FFIEC snapshot is loaded. Area context, not a finding about a borrower, household, or property. Missing and unavailable data stay distinct. |
...
| ZIP industry establishments | `locus_cbp_zip` | `{ "address": "123 Main St, Chapel Hill, NC 27514" }` | Census 2023 County Business Patterns counts by broad 2-digit NAICS industry for the address ZIP. A ZIP is area context, not the premises, customers, sales, or a location recommendation; unavailable data is not zero. |
| Area eviction filing counts | `locus_eviction_filings` | `{ "address": "..." }` or `{ "latitude": 35.78, "longitude": -78.64 }` | Published Eviction Lab weekly tract and Legal Services Corporation monthly county filing aggregates, each with a date and source. Area context only; never use to screen a tenant, landlord, household, or property. |
...
First use free `locus_tax_calendar` and `locus_appeal_filing_guide` to check the rule and filing path, then a supported `locus_property_tax_estimate` if annual tax is the question. If the owner still asks how the assessment compares and approves payment, use `POST /api/locus-assessment-position { "address": ..., "noticeDate": optional }` ($0.10).
...
support, and writes one hedged summary for the audience the question implies (the response's
`questionUse` shows the persona signal and focus group the question resolved to; the question
text itself is never echoed in the response). Not every
